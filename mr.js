...
function mrInit(callerGlobalThis){
   ...
   */
	...
	const sleep = millis => {
		assert(Utilities && is(Utilities.sleep, Function));
		log(`Sleeping for ${millis}ms`);
		return Utilities.sleep(millis);
	};//sleep
   ...
   */
	...
	if(!luxonInit) throw new ReferenceError('Missing dependency: luxon');
	const { DateTime, /*Duration,*/ Interval } = luxonInit({});
	...
	// const Cache = Object.defineProperties({}, ['Document', 'Script', 'User'].reduce((cache, name) =>
	//   Object.assign(cache, { [name]: {
	//     // enumerable: false,//must remain false or else we will inadvertently access these getters
	//     get: () => {
	//       log(`Cache.${name} getter invoked!`);
	//       const service = retryGoogle(() => CacheService[`get${name}Cache`]());
	//     }//get
	//   }}), {}//Object.assign
	// ));//Object.defineProperties
	...
	const Lock = Object.defineProperties({}, [ 'Document', 'Script', 'User' ].reduce((lock, name) =>
   ...
   */
	...
	function testInclude(){
		log(HtmlService.createTemplateFromFile('testinclude').evaluate().getContent());
	}
	...
	const user = Session && retryGoogle(() => Session.getActiveUser().getEmail());
	const url = ScriptApp && retryGoogle(() => ScriptApp.getService().getUrl());
   ...
   */
	...
	if(globalThis.window) globalThis.window.addEventListener('DOMContentLoaded', _karosInit);
