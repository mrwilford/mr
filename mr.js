...
const mrExecutionBegin = new Date();//in case create() is not called immediately
function mrCreate(scope){
   ...
   */
  const retry = (func, { tries = 1, wait = mr.secToMs(1), check, onFail = retryOnFailDefault } = {}) => {
    for (let tryNum = 1; true; ++tryNum) {
      try { return func(tryNum) }//may fail
      catch (err) {
        const doneTrying = tries <= tryNum || is.func(check) && !check(err, tryNum);
        if (doneTrying) return is.func(onFail) && onFail(err, tryNum);
        log.warn('mr.retry: attempt number ' + tryNum + ' of ' + tries + ' has failed:\n' + err.message);
        sleep(wait);//wait before trying again
      }//catch
    }//for
  }//retry
  const retryOnFailDefault = (err) => { throw err };
  ...
  const is = function(v){
    ...
    if (is.ss(v)) return server.drive.ss;
    if (is.sheet(v)) return server.drive.sheet;
    if (is.form(v)) return server.drive.form;
    if (is.doc(v)) return server.drive.doc;
    if (is.slide(v)) return server.drive.slide;
    if (is.site(v)) return server.drive.site;
    if (is.page(v)) return server.drive.page;
    if (is.file(v)) return server.drive.file;
    if (is.folder(v)) return server.drive.folder;
  ...
  const object = (...v) => {
    ...
    return retry(() => {
      assert(!is.prim(v[0]));
      return Object.assign({}, ...v.map(x => Object.fromEntries(x)));
    }, {
      onFail: () =>
        Object.fromEntries([...v.map((x, i) => [i, x]).concat(v.length ? [['length', v.length]] : [])])
    });//attempt
  };//object
  const obj = object;
  ...
  const isClient = 'undefined'!=typeof window;//do we have a browser window?
  const server = isClient ? undefined : {};
  const client = isClient ? {} : undefined;
  if(isClient){
    ...
    assert($ && document && M);//we depend on jquery and materialize
    client.get = domSelectStr => [...document.querySelectorAll(domSelectStr)];
    client.init = () => {
      //init MATERIALIZECSS and collapse all collapsibles
      M.AutoInit();
      // $('.collapsible.expandable').each(x => M.Collapsible.init(x[0], { accordion: false }));
      client.get('.collapsible.expandable').forEach(x => M.Collapsible.init(x[0], { accordion: false }));
      //ensure unique ids
      const ids = client.get('*').map(x => ({ id: x.id, count: 0 }));
      client.get('*').forEach(x => {
        const matches = ids.filter(y => y.id === x.id);
        matches.uno;//if(matches.length) this.id += (matches.uno.count++).toString().pad(4);
      });//each
      //attach events
      client.get('*').forEach(x => x.getAttribute('data-mr'));
    }//init
    client.goto = function (args, dom) {
      var main = client.get('main').uno;
      var href = client.get('#' + dom.attr('href').replace(/\#/g, ''));
      ...
      //FIX ME: client.get('ul.tabs').tabs('select', tab.attr('id'));//goto tab
    ...
    client.getProps = function (dest) {
      client.get('#' + dest).innerHTML = 'Getting properties...';
      run({
        req: 'propAll', success: function (props) {
          ...
          var newHtml = props.reduce(function (a, x) {
            ...
            return a + '<div><code>[' + x.type + ']</code><a href="##" class="link small" onclick=\'run({' +
              ...
              '    args:"' + x.prop + '",' +
              '    success:function(v){if(v)client.getProps("' + dest + '")}' +
              '})\'> ' + x.prop + ' </a></div>';
          }, '');//reduce
          client.get('#' + dest).innerHTML = newHtml;
        }//success
      });//run
    }//getProps
    ...
    client.toggle = function (args) {
      ...
      descend(client.get('#' + args.id));
    }//toggle
    // client.filter = function (args, dom) {
    //   var filter = $('#' + args.filter).val().toUpperCase();
    //   var $select = $('#' + args.select);//$('#'+args.select);
    //   var $children = $select.children();
    //   var placeholder = $children.first().text().trim();
    //   $select.val(placeholder);
    //   if (args.disable) $('#' + args.disable).prop('disabled', true);
    //   $children.each(function (x) {
    //     var $this = $(this);
    //     var text = $this.text().toUpperCase();
    //     if (0 > text.indexOf(filter)) $this.hide();
    //     else $this.show();
    //   });//each
    //   //run({req : 'print', args : instDisplay});
    // }//filter
    client.simulateReg = function (args, dom) {
      run({
        req: 'simulateRegs', args: args, success: function (x) {
          client.get('#txtTestNumRegs').value = 1;
          M.updateTextFields();//refresh any text fields
        }//success
      });//run
    }//simulateReg
    client.checkRegForm = function (args, dom) {
      const status = client.get('#spnStatusRegForm');
      status.innerHTML = statusErr + 'Out of date';
      status.className = 'error';
    }//checkRegForm
    //schedule our init to run when the document is ready
    ...
    assert(DriveApp && Sheets && server);//we depend on the apis: Drive, Sheets
    ...
    server.drive = (desc) => {
      ...
      if(desc===undefined) return server.drive.active;
      if(is.str.id(desc)) return server.drive(profile(() => DriveApp.getFileById(desc),'DriveApp.getFileById'));
      if(is.str.url(desc)) return server.drive(server.drive.urlToId(desc));
      if(is.str(desc)) return server.drive(server.drive.byPath(desc));
      const type = is.drive(desc);
      assert(is.obj(desc) && type && type.mime);
      if(desc.mr) return desc;//an mr.drive.file was passed in
      const id = desc.getId();
      const isDerived = DriveApp!==type.creator;
      if(isDerived) return server.drive(DriveApp.getFileById(id));
      if(!server.drive.cache) server.drive.cache = {};
      if(server.drive.cache[id]) return server.drive.cache[id];//cache hit
      let mrDoc = {mr:{}};
      let gFile = desc;
      ...
      switch(mrDoc.type){
        ...
        case MimeType.FOLDER:
          obj.cacheable(mrDoc,'files'  ,() => loop(gFile.getFiles  (),f => server.drive(f.elem)).elems);
          obj.cacheable(mrDoc,'folders',() => loop(gFile.getFolders(),f => server.drive(f.elem)).elems);
          mrDoc.get = desc => {
            ...
            return result.map(x => server.drive(x));
          }//get
          break;
        ...
        case MimeType.GOOGLE_SHEETS:
          ...
          const resetReadCacheables = () => {
            object.keys(batchGet).forEach(modeType => {
              ...
              obj.cacheable(mrDoc,[type.mode,...type.nicknames],() => {
                const activeReq = _readRequests.reduce((a,y) => a+(y[type.mode]||''),'').slice(0,-1);//drop trailing comma
                if(activeReq.length) profile(()=>mrDoc.read.go(type.mode),'read.go');//⏳ expensive read happens here
          ...
          const readHeadings = (sheet = null) => {
            ...
            if(!sheet || !sheet.mr.headings) profile(() => arr(mrDoc.sheets).forEach(s => {
              ...
              s.mr.headings = [ 
                ...
                ...s.getRange(s.headingRow,1,1,s.lastColumn).getValues()[0]//TODO: use batchGet to get all sheets!
          ...
          obj.cacheable(mrDoc,'sheets',() => {
            ...
            gSheets.forEach(s => {
              ...
              obj.cacheable(s,['ss','parent'],() => server.drive(s.getParent()));
          ...
          obj.cacheable(mrDoc,'headings',() => {
            const headings = {};
            arr(mrDoc.sheets).forEach(s => obj.cacheable(headings, [s.name,s.abbr,s.uppername], () => s.headings));
            return headings;
          });//cacheable headings
      ...
      return server.drive.cache[id] = mrDoc;
    };//drive

    //inherit from DriveApp and add some shortcuts and value cacheing for simple getters
    obj.extend(server.drive,DriveApp);
    obj.cacheables(server.drive);

    server.drive.auth = true;//assume we are authorized unless told otherwise
    server.drive.byPath = (path) => {
        const onFail = (n,msg) => { if(n) log.warn('Multiple '+msg); else throw new Error('No '+msg) };
        return server.drive(path.split(/[\\\/]/).reduce((a,x,i,arr) => {
          if( '.'==x) return a;
    ...
    server.drive.utilities = Utilities;
    server.drive.fileTypes = ENUM('folder','ss','doc','form','site','slide');

    server.drive.file = server.drive.File = {};

    server.drive.folder = server.drive.Folder = {};
    obj.extend(server.drive.folder,DriveApp.getRootFolder);//inherit from root folder
    obj.cacheables(server.drive.folder);//add value cacheing for simple getters
    server.drive.folder.isFileType = true;

    server.drive.ss = server.drive.spreadsheet = server.drive.Spreadsheet = {};
    obj.extend(server.drive.ss,SpreadsheetApp);//inherit from SpreadsheetApp
    obj.cacheables(server.drive.ss);//add value cacheing for simple getters
    server.drive.ss.isFileType = true;

    server.drive.sheet = server.drive.Sheet = {};
    obj.cacheables(server.drive.sheet,'active',()=>server.drive.ss.activeSheet);

    server.drive.doc = server.drive.document = server.drive.Document = {};
    obj.extend(server.drive.doc,DocumentApp);
    obj.cacheables(server.drive.doc);//add value cacheing for simple getters
    server.drive.doc.active = server.drive.doc.activeDocument;
    server.drive.doc.isFileType = true;

    server.drive.form = server.drive.Form = {};
    obj.extend(server.drive.form,FormApp);
    obj.cacheables(server.drive.form);//add value cacheing for simple getters
    server.drive.form.active = server.drive.form.activeForm;
    server.drive.form.isFileType = true;

    server.drive.slide = server.drive.presentation = server.drive.Slide = server.drive.Presentation = {};
    obj.extend(server.drive.slide,SlidesApp);
    obj.cacheables(server.drive.slide);//add value cacheing for simple getters
    server.drive.slide.active = server.drive.slide.activePresentation;
    server.drive.slide.isFileType = true;

    server.drive.site = server.drive.Site = {};
    obj.extend(server.drive.site,SitesApp);
    obj.cacheables(server.drive.site);//add value cacheing for simple getters
    server.drive.site.active = server.drive.site.activeSite;
    server.drive.site.isFileType = true;

    server.drive.page = server.drive.Page = {};
    server.drive.page.active = server.drive.site.activePage;

    obj.cacheable(server.drive,'active' , () => uno(loop(server.drive.types  , i => server.drive[+i].active).elems));
    obj.cacheable(server.drive,'files'  , () =>     loop(server.drive.files  , i => server.drive(i.elem)   ).elems) ;
    obj.cacheable(server.drive,'folders', () =>     loop(server.drive.folders, i => server.drive(i.elem)   ).elems) ;
    obj.cacheable(server.drive,'items'  , () => server.drive.folders.concat(server.drive.files));
    obj.cacheable(server.drive,'trashed', () => {
      const folders = loop(server.drive.trashFolders, i => server.drive(i.elem)).elems;
      const files   = loop(server.drive.trashFiles  , i => server.drive(i.elem)).elems;
    ...
    server.drive.prop = function(propName,setValue,propType='Document'){
      assert(Object.keys(server.drive.prop.types).filter(x=>x==propType).length);
      //log(version+' | prop('+propName+(setValue?(','+setValue):'')+')');
      if(!server.drive.auth) return null;
    ...
    obj.cacheable(server.drive,'props',server.drive.prop);
    server.drive.prop.types = ENUM('DOCUMENT','USER','SCRIPT');
