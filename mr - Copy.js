...
const mrCreate = function() {

	var mr = {};//fill this up and return it
	 ...
	 */
	mr.assert = function(test = null,onFail = new Error('❌ ASSERT FAILED!')) {
		if(!!test) return true;
		if(mr.is.func(onFail)) return onFail(test);
		if(mr.is.error(onFail)) throw onFail;
		mr.log.error(JSON.stringify(onFail));
		return false;
	};//assert
	...
	mr.created = Date.now();
	mr.configs = mr.makeEnum('RELEASE','DEBUG');
	mr.config = mr.configs.DEBUG;
	...
	mr.is = {};
	mr.is.undefined = mr.is.undef = (v) => 'undefined' == typeof v;
	mr.is.defined = mr.is.def = (v) => !mr.is.undefined(v);
	mr.is.primitive = mr.is.prim = (v) => v !== Object(v);
	mr.is.boolean = mr.is.bool = (v) => 'boolean' == typeof v;
	mr.is.nan = mr.is.NaN = (v) => Number(v) !== Number(v);
	mr.is.finite = mr.is.fin = isFinite;
	mr.is.number = mr.is.num = (v) => 'number' == typeof v;
	mr.is.object = mr.is.obj = (v) => !!(v && typeof v === 'object' && v.constructor === Object);
	mr.is.array = mr.is.arr = (v) => !!(v && typeof v === 'object' && v.constructor === Array);
	mr.is.string = mr.is.str = (v) => typeof v === 'string' || v instanceof String;
	mr.is.function = mr.is.func = (v) => !!(v && {}.toString.call(v) === '[object Function]');
	mr.is.date = mr.is.Date = (v) => !!(v && {}.toString.call(v) === '[object Date]');
	//mr.is.numeric = (v)=>!mr.is.nan(parseFloat(v)) && isFinite(v);//use isFinite instead

	//specialized string checks
	mr.is.string.id = (v) => mr.is.str(v) && 44 == v.length && mr.is.id.regex.test(v);
	mr.is.string.id.regex = new RexExp("[A-Z0-9-_]","gi");//NOTE: Specific to DriveApp
	mr.is.string.url = (v) => mr.is.str(v) && mr.is.url.regex.test(v);
	mr.is.string.url.regex = new RegExp("^(https?://)?(((www\\.)?([-a-z0-9]{1,63}\\.)*?[a-z0-9][-a-z‌​0-9]{0,61}[a-z0-9]\\‌​.[a-z]{2,6})|((\\d{1‌​,3}\\.){3}\\d{1,3}))‌​(:\\d{2,4})?((/|\\?)‌​(((%[0-9a-f]{2})|[-\‌​\w@\\+\\.~#\\?&/=])*‌​))?$","g");
	mr.is.string.mailto = (v) => mr.is.str(v) && 0 == v.indexOf('mailto:');
	 ...
	 * @return {boolean}
	 */
	mr.as.boolean = mr.as.bool = function(v,{trueStrings = ['true','yes','✔','☑','✅','👍'],isCaseSensitive = false} = {}) {
		const vCompare = isCaseSensitive ? v.trim : v.trim.lower;
		if(!!trueStrings.filter(x => vCompare === isCaseSensitive ? x.trim : x.trim.lower).length) return true;
		var nums = mr.as.numbers(v);
	...
	mr.drive = function(desc) {
		if(mr.is.str.id(desc)) return mr.drive.file(DriveApp.getFileById(desc));
		if(mr.is.str.url(desc)) return mr.drive.file(mr.drive.urlToId(desc));
		if(mr.is.str(desc)) return mr.drive.file(desc.split(/[\\\/]/).reduce((a,x,i,arr) => mr.as.arr(arr[i] ? a.getFoldersByName(x) : a.getFilesByName(x)).uno,DriveApp.getRootFolder()));
		assert(mr.is.obj(desc));
		if(desc.id) return desc;//an mr.drive.file was passed in
		if(!desc.getMimeType) return mr.drive.file(DriveApp.getFileById(desc.getId()));//a specific google file type was passed in
		const id = desc.getId();
		if(!mr.drive.cache) mr.drive.cache = {};
		if(mr.drive.cache[id]) return mr.drive.cache[id];//cache hit
		var mrFile = {},gFile = desc,gDoc = mr.drive.as(desc);
		//inherit from base and add some handy shortcuts and value cacheing for simple getters
		//first do so for generic DriveApp file type
		for(let x in gFile) {
			if(!mr.is.func(gFile[x])) {
				mrFile[x] = gFile[x];
				continue;
			}//if
			if(0 != x.indexOf('get')) {
				mrFile[x] = mr.partial({thisArg: gFile,func: x});
				continue;
			}//if
			const abbr = x.charAt(3).toLowerCase() + x.slice(4);//replace('get','').toLower(1)
			try {gFile[x]()} catch(err) {continue}
			mrFile[x] = () => {
				const value = gFile[x]();
				mr.define(mrFile,abbr,{value: value});//cache the value
				return value;
			};//function
			mr.define(mrFile,abbr,{getter: mrFile[x]});
		}//for
		//now do so for specific file type
		for(let x in gDoc) {
			if(!mr.is.func(gDoc[x])) {
				mrFile[x] = gDoc[x];
				continue;
			}//if
			if(0 != x.indexOf('get')) {
				mrFile[x] = mr.partial({thisArg: gDoc,func: x});
				continue;
			}//if
			const abbr = x.charAt(3).toLowerCase() + x.slice(4);//replace('get','').toLower(1)
			try {gDoc[x]()} catch(err) {continue}
			mrFile[x] = () => {
				const value = gDoc[x]();
				mr.define(mrFile,abbr,{value: value});//cache the value
				return value;
		...
		return mr.drive.cache[id] = mrFile;
	};//drive

	mr.drive.is = {};
	mr.drive.is.ss = (f) => MimeType.GOOGLE_SHEETS === f.getType();
	mr.drive.is.doc = (f) => MimeType.GOOGLE_DOCS === f.getType();
	mr.drive.is.form = (f) => MimeType.GOOGLE_FORMS === f.getType();
	mr.drive.is.slide = (f) => MimeType.GOOGLE_SLIDES === f.getType();
	mr.drive.is.folder = (f) => MimeType.FOLDER === f.getType();
	//TODO: add other types here

	mr.drive.as = (f) => Object.keys(mr.drive.is).map(x => mr.drive.is[x](f) ? mr.drive.as[x](f) : 0).filter(x => x).uno;
	mr.drive.as.ss = (f) => SpreadsheetApp.openById(f.getId());
	mr.drive.as.doc = (f) => DocumentApp.openById(f.getId());
	mr.drive.as.form = (f) => FormApp.openById(f.getId());
	mr.drive.as.slide = (f) => SlideApp.openById(f.getId());
	mr.drive.as.folder = (f) => DriveApp.getFolderById(f.getId());

	mr.drive.urlToId = (url) => url.match(/[-\w]{25,}/)[0];
...
if(globalThis.mr) throw new Error('mr is already defined!');
Object.defineProperty(globalThis,'mr',{
	getter: (function create() {
		var mr = {};
		mr.clone = create;//allow mr.clone() to produce another mr


		Object.defineProperty(globalThis,'mr',{value: mr});
		return mr;
	})
});//defineProperty
...
function mrImport(scope = globalThis) {
	...
	////////////////////////////////
	{
		...
		mr.as.boolean =
			mr.as.bool =
			function(v,{trueStrings = ['true','✔'],isCaseSensitive = false} = {}) {
				const vCompare = isCaseSensitive ? v.trim : v.trim.lower;
				if(!mr.is.str(v)) return !!v;
				return !!trueStrings.filter(x => vCompare === isCaseSensitive ? x.trim : x.trim.lower).length;
			}; //as.boolean
	...
	////////////////////////////////
	{
		 ...
		 */
		mr.assert =
			function(test = null,onFail = 'assert failed: ' + test) {
				if(test) return true;
		 ...
		 */
		mr.profile =
			function(func,desc = _caller + '.' + func.name,thisArg) {
		 ...
		 */
		mr.retry =
			function({funcTry,num = 4,wait = 100,check,thisArg}) {
		 ...
		 */
		mr.poll =
			function({func,allowed = Infinity,runEvery = 100,thisArg}) {
		 ...
		 */
		mr.duration =
			function(start = mr.entry,end = Date.now()) {
				let result = {};
				...
				mr.define(result,['description','desc'],{
					get: () => {
		 ...
		 */
		mr.getLoremIpsum =
			function(url = 'https://loripsum.net/api/',...options) {
		...
		mr.define(mr,'_callstack',{
			getter: () => {
				try {
					...
					matches = matches.map(x => ({ //e.g: "test (mr:1824:3)"
	...
	{
		mr.log = function(msg) {
			if('RELEASE' != mr.config && mr.log.useLogger) Logger.log(msg);
			if('RELEASE' != mr.config && mr.log.useConsole) console.log(msg);
			return msg;
		};
		mr.log.info = function(msg) {
			if('RELEASE' != mr.config && mr.log.useLogger) Logger.log('*INFO* ' + msg);
			if('RELEASE' != mr.config && mr.log.useConsole) console.info(msg);
			return msg;
		};
		mr.log.warn = function(msg) {
			if('RELEASE' != mr.config && mr.log.useLogger) Logger.log('**WARNING** ' + msg);
			if('RELEASE' != mr.config && mr.log.useConsole) console.warn(msg);
			return msg;
		};
		mr.log.error = function(msg) {
			if('RELEASE' != mr.config && mr.log.useLogger) Logger.log('***ERROR*** ' + msg);
			if('RELEASE' != mr.config && mr.log.useConsole) console.error(msg);
			return msg;
		};
	...
	////////////////////////////////
	{
		 ...
		 */
		mr.drive.email =
			function({to,cc,bcc = mr.drive.bccAllEmails,from = {noReply: true,name: 'AUTOMATED EMAILER',email: Session.getActiveUser().getEmail()},subject = '',body,isTest = false}) {
				...
				try {
					...
					const mailQuota = mr.profile(() => MailApp.getRemainingDailyQuota(),'getQuota');
	...
	////////////////////////////////
	{
		...
		mr.drive.file = function(_file) {
			...
			for(let x in _file) {
				this[x] = mr.is.func(_file[x]) ? mr.partial({thisArg: _file,func: x}) : _file[x]; //inherit from base
			}//for
		 ...
		 * @return {string}
		 */
		mr.drive.file.prototype.getUrl =
			function() {
		 ...
		 * @return {string}
		 */
		mr.drive.file.prototype.getName =
			function() {
		 ...
		 * @return {string}
		 */
		mr.drive.file.prototype.getDownloadUrl =
			mr.drive.file.prototype.getLink =
			function() {
		 ...
		 * @return {number}
		 */
		mr.drive.file.prototype.getSize =
			function() {
		 ...
		 * @return {string}
		 */
		mr.drive.file.prototype.getDescription =
			mr.drive.file.prototype.getDesc =
			function() {
		 ...
		 * @return {Date}
		 */
		mr.drive.file.prototype.getDateCreated =
			mr.drive.file.prototype.getCreated =
			function() {
		 ...
		 * @return {Date}
		 */
		mr.drive.file.prototype.getLastUpdated =
			mr.drive.file.prototype.getUpdated =
			function() {
		 ...
		 * @return {Object}
		 */
		mr.drive.file.prototype.getSharing =
			function() {
				const sharing = {
		 ...
		 * @return {[drive.folder]}
		 */
		mr.drive.file.prototype.getParents =
			function() {
		 ...
		 */
		mr.drive.file.prototype.property =
			mr.drive.file.prototype.prop =
			function(propName,setValue) {
	...
	////////////////////////////////
	{
		...
		mr.drive.folder = function(_folder) {
			...
			Object.defineProperty(this,'_folder',{
				value: _folder
			}); //make our private member public but read-only
			for(let x in _folder) {
				this[x] = mr.is.func(_folder[x]) ? partial({thisArg: _folder,func: x}) : _folder[x]; //inherit from base
			}//for
		 ...
		 * @return {[drive.folder]}
		 */
		mr.drive.folder.prototype.getFolders =
			function() {
	...
	////////////////////////////////
	{
		...
		mr.drive.ss =
			function(_ss) {
				...
				for(let x in _ss) {
					this[x] = mr.is.func(_ss[x]) ? partial({thisArg: _ss,func: x}) : _ss[x]; //inherit from base
				}//for
				//add new ss to cache
		 ...
		 */
		mr.drive.ss.prototype.getSheets =
			function() {
		 ...
		 */
		mr.drive.ss.prototype.get =
			function(mode = 'FORMULA',requests = []) {
				...
				if(mr.drive.ss.useSheetsAPI) {
					var reqs = requests.map(x,i => ({value: x,index: i})).filter(x => x.value.request.has('!'));
		 ...
		 */
		mr.drive.ss.prototype.read =
			function({mode = 'FORMATTED_VALUE',sheets = this.sheets}) {
				sheets.forEach((x,i) => {
					let headings;
					...
					if(!mr.drive.ss.useSheetsAPI) headings = s.properties.sheet.getRange('1:1').getValues()[0];
						...
						const headingsResponse = this.batchGet({requests: "'" + s.name + "'!1:1",mode: 'FORMATTED_VALUE'});
		...
		mr.drive.ss.prototype.read =
			function(args) {
				...
				var sheets = null;
				if(isUndefined(args) || isUndefined(args.sheets)) sheets = this.properties.sheets.map(x => ({name: x.properties.name}));
				...
				for(i = 0; i < sheets.length; ++i) {
					...
					/*headings = */
					response.valueRanges.shift().values[0]; //get the 1:1 request
					...
					cols.forEach((y,j) => {
						...
						responseValues = response.valueRanges.shift().values.map(z => z[0]);
			...
			function(args) {
				...
				for(i = 0; i < args.sheets.length; ++i) {
					...
					if(!cols) {
						...
						curSheet.columns = [];
						s.properties.colsRead.forEach(y => curSheet.columns.push({index: y}));
					...
					if(!isArray(cols)) {
						//treat object as if it were an array
						if(isObject(cols)) cols = Object.keys(cols).map(x => cols[x]);
						else cols = [cols];
					} //if
	...
	////////////////////////////////
	{
		...
		mr.drive.ss.sheet =
			function(_sheet) {
				...
				for(let x in _sheet) {
					this[x] = mr.is.func(_sheet[x]) ? partial({thisArg: _sheet,func: x}) : _sheet[x]; //inherit from base
				}//for
			}; //drive.ss.sheet
		 ...
		 */
		mr.drive.ss.sheet.prototype.getColumn =
			mr.drive.ss.sheet.prototype.getCol =
			function(_column) {
		 ...
		 * @return drive.ss
		 */
		mr.drive.ss.sheet.prototype.getSpreadsheet =
			mr.drive.ss.sheet.prototype.getSS =
			mr.drive.ss.sheet.prototype.getParent =
			function() {
		 ...
		 * @return string
		 */
		mr.drive.ss.sheet.prototype.getSheetId =
			mr.drive.ss.sheet.prototype.getId =
			function() {
		 ...
		 * @return string
		 */
		mr.drive.ss.sheet.prototype.getName =
			function() {
		 ...
		 */
		mr.drive.ss.sheet.prototype.getRows =
			function() {
				const rows = {
		 ...
		 * @return {max:number,last:number}
		 */
		mr.drive.ss.sheet.prototype.getCols =
			function() {
				const cols = {
		 ...
		 * @return drive.form
		 */
		mr.drive.ss.sheet.prototype.getForm =
			function() {
		 ...
		 */
		mr.drive.ss.sheet.prototype.read =
			function(args) {
				args = args || {};
		 ...
		 * @param columnReference : {letter:string | number:number | heading:string}
		 */
		mr.drive.ss.sheet.prototype.write =
			function(args) {
				args = args || {};
				if(mr.is.arr(args)) args = {columns: args};
				args.name = this.name;
		 ...
		 * @return Range
		 */
		mr.drive.ss.sheet.prototype.get =
			function(args) {
		 ...
		 */
		mr.drive.ss.sheet.prototype.deleteRows =
			function(indices) {
		 ...
		 */
		mr.drive.ss.sheet.prototype.updateResponseURLs =
			function(args) {
				args = args || {};
				...
				for(i = 1; i < numRows; ++i) {
					...
					if(0 <= oldUrl.indexOf('<< NOT FOUND')) {
						if(0 > newUrl.indexOf('<< NOT FOUND')) log({
							msg: 'Response was found but it used to not exist!'
						});
					} else if('' === oldUrl) {
						if(0 <= newUrl.indexOf('<< NOT FOUND')) log({
							msg: 'Response cannot be found'
						});
					} else {
						if(0 <= newUrl.indexOf('<< NOT FOUND')) log({
							msg: 'Response used to exist but can no longer be found!'
						});
					} //else
					urls.set(i,newUrl);
				} //for
	...
	////////////////////////////////
	{
		...
		////////////////////////////////
		{
			...
			mr.api.sheets.quotas.read.throttle = mr.partial({
				func: mr.api.sheets.throttle,
				args: [mr.api.sheets.quotas.read]
			});
			...
			mr.api.sheets.quotas.write.throttle = mr.partial({
				func: mr.api.sheets.throttle,
				args: [mr.api.sheets.quotas.write]
			});
			mr.api.sheets.quotas.write.freq = 1000; //milliseconds between requests
			mr.api.sheets.quotas.write.last = null;
			mr.api.sheets.throttle = (quota,requests) => {
			 ...
			 * @return Response
			 */
			mr.api.sheets.batchGet =
				function({id,ranges,valueRenderOption = 'FORMULA',majorDimension = 'ROWS',dateTimeRenderOption = 'SERIAL_NUMBER'}) {
			 ...
			 * @return Response
			 */
			mr.api.sheets.batchUpdate =
				function({id,requests}) {
	...
	////////////////////////////////
	{
		...
		Object.defineProperty(Array.prototype,'count',{
			value: (value) => this.filter(x => x === value).length
		});
		...
		Object.defineProperty(Array.prototype,'uno',{
			value: (onFail) => this.expect(1,onFail)[0]
		});
		...
		Object.defineProperty(Array.prototype,'expect',{
			value: (num,onFail) => {
		...
		Object.defineProperty(Array.prototype,'filterIndex',{
			value: (func,thisArg) => this
				.map((x,i) => ({value: x,index: i}),thisArg)
				.filter((x,i) => func(x.value,i),thisArg)
				.map(x => x.index,thisArg)
		}); //filterIndex
		...
		Object.defineProperty(Array.prototype,'random',{
			value: (start = 0,exclude = []) => this[mr.rng.int({
				min: start,
				max: this.length,
				exclude: exclude
			})]
		 ...
		 * @return *
		 */
		Object.defineProperty(Array.prototype.random,'gaussian',{
			value: (start = 0,exclude = []) => this[mr.rng.int({
				min: start,
				max: this.length,
				exclude: exclude
		...
		Array.prototype.union = (operand) => [...new Set([...this,...operand])];
		...
		Array.prototype.intersect = (operand) => [...new Set([...this.filter(x => operand.has(x))])];
		...
		Array.prototype.subtract = (operand) => [...new Set([...this.filter(x => !operand.has(x))])];
		...
		Object.defineProperty(Array.prototype,'mux',{
			value: (operand,...moreOperands) => operand ? this.cartesian(operand).mux(...moreOperands) : this
		}); //mux
		...
		Object.defineProperty(Array.prototype,'cartesian',{
			value: (operand) => [].concat(...this.map(x => operand.map(y => [].concat(x,y))))
		}); //cartesian
		...
		Object.defineProperty(Array.prototype,'mmult',{
			value: (operand) => {
				let result = [];
				this.forEach((x,i) => {
					result[i] = [];
		...
		Object.defineProperty(Array.prototype,'transpose',{
			value: () => {
				...
				for(let i = 0; i < h; ++i) {
					...
					for(let j = 0; j < w; ++j) {
						t[i][j] = this[j][i]; //Save transposed data
					} //for
				} //for
				return t;
			}
		...
		Object.defineProperty(Array.prototype,'cut',{
			value: (pos,len) => this.substring(0,pos) + this.substring(pos + len)
		}); //cut
	}
	...
	////////////////////////////////
	{
		...
		Object.defineProperty(String.prototype,'isBlank',{get: () => '' === this.trim()});

		if(String.prototype.lower) mr.log.warn('String.prototype.lower ALREADY EXISTS & IS BEING OVERWRITTEN');
		Object.defineProperty(String.prototype,'lower',{get: () => this.toLowerCase()});

		if(String.prototype.upper) mr.log.warn('String.prototype.upper ALREADY EXISTS & IS BEING OVERWRITTEN');
		Object.defineProperty(String.prototype,'upper',{get: () => this.toUpperCase()});

		if(String.prototype.camel) mr.log.warn('String.prototype.camel ALREADY EXISTS & IS BEING OVERWRITTEN');
		Object.defineProperty(String.prototype,'camel',{get: () => this.toLowerCase().replace(/(?:^\w|[A-Z]|\b\w)/g,x => i ? x.toLowerCase() : x.toUpperCase()).replace(/\s+/g,'')}); //camel

		if(String.prototype.count) mr.log.warn('String.prototype.count ALREADY EXISTS & IS BEING OVERWRITTEN');
		Object.defineProperty(String.prototype,'count',{value: (search) => (this.match(new RegExp(search,'g')) || []).length});

		if(String.prototype.has) mr.log.warn('String.prototype.has ALREADY EXISTS & IS BEING OVERWRITTEN');
		Object.defineProperty(String.prototype,'has',{value: (search) => 0 < this.count(search)});
		...
		Object.defineProperty(String.prototype,'findClosing',{
			value: (pos) => {
		...
		Object.defineProperty(String.prototype,'distance',{value: (operand,options = {}) => this.smithWaterman(operand,options)});
		...
		Object.defineProperty(String.prototype,'levenshtein',{
			value: (operand) => {
				...
				const distanceMatrix = Array(b.length + 1).fill(null).map(x => Array(a.length + 1).fill(null));
				...
				for(let j = 1; j <= b.length; ++j) {
					for(let i = 1; i <= a.length; ++i) {
						...
						distanceMatrix[j][i] = Math.min(
							distanceMatrix[j - 0][i - 1] + 1, //deletion
							distanceMatrix[j - 1][i - 0] + 1, //insertion
		...
		Object.defineProperty(String.prototype,'smithWaterman',{
			value: (operand,options = {}) => {
				localAligner = (arrayQ,arrayP,options) => {
					...
					this._matrixA = arrayP.concat([0]).map(() => arrayQ.concat([0]).map(y => 0));
					this._matrixB = arrayP.concat([0]).map(() => arrayQ.concat([0]).map(y => 0));
					this._finalQ = [];
					this._finalP = [];
					this._maxScore = 0;
					this._maxI = 0;
		...
		Object.defineProperty(String.prototype,'closest',{
			value: (list,options = {}) => {
		...
		Object.defineProperty(String.prototype,'common',{
			value: (operand) => {
				const X = this;
				const Y = operand;
				const m = X.length;
	...
	////////////////////////////////
	{
		...
		Object.defineProperty(Number.prototype,'rescale',{
			value: (domain = {min: 0,max: 1},range = {min: 0,max: 1}) => {
		...
		Object.defineProperty(Number.prototype,'normalize',{
			value: ({min = 0,max = 1} = {}) => this.rescale({min: min,max: max},{min: 0,max: 1})
		});
		...
		Object.defineProperty(Number.prototype,'gaussian',{
			value: ({min = 0,max = 1} = {}) => this.boxMuller({min: min,max: max})
		});
		...
		Object.defineProperty(Number.prototype,'boxMuller',{
			value: ({min = 0,max = 1} = {}) => {
				let v = 0;
